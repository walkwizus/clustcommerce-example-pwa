...
class Product extends React.Component {
  ...
  render() {
    if (this.state.product.type_id === "bundle") {
      return <BundleProduct product={this.state.product} {...this.state.props} />
    } else {
      return <SimpleProduct product={this.state.product} {...this.state.props} />
    }
  }
}

/**
 * Simple product with or without options
 */
class SimpleProduct extends React.Component {
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    ...
    nextProps.product.options.forEach((option) => {
      option.values.forEach((optionValue) => {
        if (optionValue.price_type === 'percent') {
          optionValue.price = nextProps.product.price * optionValue.price / 100;
        }
      })
    })

    this.setState({'product': nextProps.product, 'price': nextProps.product.price.toLocaleString(navigator.language || navigator.userLanguage, {style: 'currency', currency: this.state.config.base_currency_code})});
  }
            ...
            <div className="col-sm-6">
              <div className="box">
                ...
                <p className="text-center buttons">
                  ...
                  {this.state.product.extension_attributes && (!this.state.product.extension_attributes.stock_salable || this.state.product.extension_attributes.stock_salable.quantity > 0 || !this.state.product.extension_attributes.stock_salable.manage_stock) ? (<button type="submit" className="btn btn-primary" id="add-to-cart" onClick={this.addToCart.bind(this)}><i
