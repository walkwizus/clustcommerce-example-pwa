...
export default class OrderNavigation extends React.Component {
  ...
  render() {
    return <ul className="nav nav-pills nav-justified">
      ...
      <li className={(this.state.index === 2 ? 'active' : (this.state.index < 2 ? 'disabled' : ''))}><Link to={"/checkout-shipping"} onClick={(e) => { if (this.state.index <= 2) { e.preventDefault(); }}}><i className="fa fa-truck"></i><br/>Delivery Method</Link></li>
